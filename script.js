...
function addNewPurch(arr, product, amount ) {
    const good = arr.find((item) => item.product === product);

    if(good && !good.isBought) {
        good.amount += amount;
    } else if(good && good.isBought) {
        good.amount = amount;
        good.isBought = false;
    } else {
        arr.push({ product, amount, isBought: false});
    }
}

addNewPurch(shoppingList, "bread", 1);
addNewPurch(shoppingList, "fish", 3);
addNewPurch(shoppingList, "water", 5);
sortByIsBought(shoppingList);
...
function buyProduct(arr, product) {
    arr.find((item) => {
...
function newPurch(good, amount, price) {
...
newPurch("laptop", 3, 2500);
newPurch("smartphone", 4, 2000);
newPurch("tv", 1, 5000);
newPurch("headphones", 2, 500);
newPurch("xbox", 1, 3000);


// /*Распечатка чека на экран;*/

function printChecks(arr) {
    arr.forEach((item) => {
        document.write(`Товар: ${item.good}<br>Количество: ${item.amount}<br>Цена за штуку: ${item.price}<br><hr>`);
    });
}

printChecks(purchasesList);
...
function getTotalCost(arr) {
    totalPrice = arr.reduce((sum, current) => sum + (current.price * current.amount),0);
    document.write(`Сумма покупки: ${totalPrice}<br><hr>`);
}

getTotalCost(purchasesList);
...
function getAverageCost(arr) {
    let mostExpensGood = arr.sort((a, b) => a.price > b.price ? -1 : 1);
    document.write(`Самый дорогой товар в чеке: ${mostExpensGood[0].good}<br>Цена: ${mostExpensGood[0].price}<br><hr>`);
}

getAverageCost(purchasesList);


// /*найвища ціна за покупку(ціна*кількість)*/

function getAverageCostByAmount(arr) {
    let mostExpensPurch = arr.sort((a, b) => a.price*a.amount > b.price*b.amount ? -1 : 1);
...
getAverageCostByAmount(purchasesList);


// /*Подсчет средней стоимости одного товара в чеке.*/

function getOnePurchAverageCost(arr) {
    let goodsAmount = arr.reduce((sum, current) => (sum + current.amount),0);
    document.write(`Средняя стоимость одного товара: ~${Math.round(totalPrice/goodsAmount)}<br><hr>`);
}

getOnePurchAverageCost(purchasesList);
...
function getRoomByFacl(arr, facultyName) {
...
getRoomByFacl(classroomsList, "mathematics");
getRoomByFacl(classroomsList, "physics");
